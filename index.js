...
function getArtistByIndex(arr, desiredIndex) {
  return `the artist at index ${arr[desiredIndex].id} is ${arr[desiredIndex].name}`;
}
...
function listOfNames(arr) {
  const copyofArtists = [...arr];
  for(let i = 0; i < arr.length; i++){
    copyofArtists[i] = arr[i].name;
  }
...
function removeArtist(arr, desiredIndex){
  const copyofArtists = [...arr];
  copyofArtists.splice(arr[desiredIndex], 1);
  return copyofArtists;
}
...
function addArtist(arr,newName,newYears,newGenre,newNation,newBio){
  arr.push({name: newName, years: newYears, genre: newGenre, nationality: newNation, bio: newBio });
  return arr;
}
...
function artistByCountry(arr, artistNat){
  const countryFilter = [];
  for(let i = 0; i < arr.length; i++){
    if(arr[i].nationality === artistNat){
      countryFilter.push(arr[i].name);
    }
  }
